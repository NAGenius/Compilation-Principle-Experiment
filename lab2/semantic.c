...
int typeEqual(Type a, Type b) {
    ...
    // <FILL_IN_HERE>
        ...
        if (a->structure->head == NULL && b->structure->head == NULL) { // 空结构体相等
            return 1;
        }
        FieldList aFields = a->structure->head;
        FieldList bFields = b->structure->head;
        while (aFields != NULL && bFields != NULL) { // 元素类型、数量比较
        ...
        return aFields == NULL && bFields == NULL;
    }
...
void check() {
    ...
    while (currentLayer != NULL) {
        Entry sym = currentLayer->layerNext;
        while (sym != NULL) {
            if (sym->type->kind == ENUM_FUNC && sym->type->func->hasDefined == 0) { // 是函数(表示已声明)并且未定义
                printf("Error type 18 at line %d: Undefined function \"%s\".\n", sym->type->func->lineno, sym->name);
            }
            sym = sym->layerNext;
        }
        currentLayer = currentLayer->hashNext;
    }
}
...
void ExtDef(Node* root) {
    ...
    if (strcmp(root->children[1]->name, "FunDec") == 0) {
        ...
        if (sym != NULL) {
            ...
            // 如果不冲突，需要为已声明的函数添加定义
            if (sym->type->func->hasDefined == 0) { // 函数未定义
                    //   <FILL_IN_HERE>
                if(!typeEqual(sym->type, newType)) { // 新函数的类型是否与已有函数类型相同
                    printf("Error type 19 at line %d: Inconsistent declaration of function \"%s\".\n", root->lineno, sym->name);
                    return;
                }
                // printf("222222222222\n"); // // test_19进入一次
...
Type Exp(Node* root) {
    ...
    if (strcmp(root->children[0]->name, "Exp") == 0) {
        ...
        // 赋值操作
        else if (strcmp(root->children[1]->name, "ASSIGNOP") == 0) { // "ASSIGNOP": 表示赋值操作
        // <FILL_IN_HERE>
            ...
            // 检查左值是否合法
            if (left->childNum != 1 ||
                (strcmp(left->children[0]->name, "ID") != 0 &&
                strcmp(left->children[0]->name, "DOT") != 0 &&
                strcmp(left->children[0]->name, "LB") != 0)) {
                // "ID": 变量
                // "DOT": 结构体
                // "LB": 数组
